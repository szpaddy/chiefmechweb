...
public class FetchURL {
	...
	private void genPostRequest(HttpRequestBase httpBase) {
		if ((httpBase instanceof HttpPost)) {
			try {
				...
				// if no binary data to upload, only text field
				...
				// upload both binary and normal text field
				if (this.binaryList.size() > 0) {
					MultipartEntity entity = new MultipartEntity();
					Iterator<BinaryData> binaryIter = this.binaryList
							.iterator();
					int size = 0;
					while (binaryIter.hasNext()) {
						...
						if (this.isTrunacated) {
							if (size + bd.getPostData().length > MAX_SEND_DATA) {
								bab = new ByteArrayBody(bd.getPostData(),
										bd.getFileName());
	...
	public void setHttpAuth(String username, String password) {
		String content = new StringBuilder().append(username).append(":")
				.append(password).toString();
		String encodeContent = encodeBase64(content);
		this.addHttpHeaders.put("Authorization",
				new StringBuilder().append("Basic ").append(encodeContent)
						.toString());
	}
	...
	public void setPostData(BinaryData data) {
		...
		this.isPost = true;
		if (logger.isDebugEnabled())
			logger.debug(new StringBuilder().append("post data:").append(data)
					.toString());
	}

	public String encodeBase64(String encodeContent) {
...
class BinaryData {
	...
	public String toString() {
		return "file name=" + this.fileName + ", parameter name="
